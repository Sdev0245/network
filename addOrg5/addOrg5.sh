...
function generateOrg5() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_CA_ORG5 up -d 2>&1
  ...
  echo
  echo "Generate CCP files for Org5"
  ./ccp-generate.sh
}
...
function generateOrg5Definition() {
  ...
  echo "##########################################################"
  echo "#######  Generating Org5 organization definition #########"
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   ...
   set +x
   if [ $res -ne 0 ]; then
     echo "Failed to generate Org5 config material..."
     exit 1
   fi
  echo
}
...
function Org5Up () {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    IMAGE_TAG=${IMAGETAG} docker-compose -f $COMPOSE_FILE_ORG5 -f $COMPOSE_FILE_COUCH_ORG5 up -d 2>&1
  else
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_ORG5 up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to start Org5 network"
    exit 1
  fi
}
...
function addOrg5 () {
  ...
  echo "###############################################################"
  docker exec Org5cli ./scripts/org5-scripts/step1org5.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo
  echo "###############################################################"
  echo "############### Have Org5 peers join network ##################"
  echo "###############################################################"
  docker exec Org5cli ./scripts/org5-scripts/step2org5.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to have Org5 peers join network"
    exit 1
  fi
...
COMPOSE_FILE_COUCH_ORG4=docker/docker-compose-couch-org5.yaml
# use this as the default docker-compose yaml definition
COMPOSE_FILE_ORG4=docker/docker-compose-org5.yaml
# certificate authorities compose file
COMPOSE_FILE_CA_ORG4=docker/docker-compose-ca-org5.yaml
# default image tag
IMAGETAG="latest"
# database
DATABASE="leveldb"
...
if [ "$MODE" == "up" ]; then
  echo "Add Org5 to channel '${CHANNEL_NAME}' with '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
